...
<form id="myform" class="layui-form" lay-filter="form" enctype="multipart/form-data">
    <table id="tableax" class="layui-table " style="display: none;">
        ...
        <tr>
            <td>用户类型：</td>
            <td>
                <select id="manager_slcata" name="cata">
                    <option value=""></option>
...
<script>
    ...
    layui.use(['table', 'layer', 'laydate', 'element', 'form'], function () {
        ...
        form = layui.form;
        //绑定添加角色按钮事件
        $.post("<%=basePath%>role/getlist", {"pn": "1", "ps": "100"}, function (data, status) {
            if (status == "success") {
                if (data != "") {
                    var json = [];
                    if (data != "") {
        ...
        tableIns = table.render({
            ...
            cols: [[
                ...
                ,
                {
                    ...
                    templet: function (d) {
                        if (d.cata == "0") {
                            return "<span style='color:#0BB20C'>管理员</span>";
                        }
                        else {
                            return "普通用户";
                        }
        ...
        table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'xgmm') {
                ...
                $.post("<%=basePath%>manager/getsingle", {"pguid": trdata.pguid}, function (data, status) {
                    if (status == "success") {
                        if (data != "") {
    ...
    function managerRole(pguid) {
        ...
        $.post("<%=basePath%>role/getlistbypguid", {"pguid": pguid}, function (data, status) {
            if (status == "success") {
                if (data != "") {
                    var json = $.parseJSON(data);
                    for (var i = 0; i < json.length; i++) {
    ...
    function roleadd() {
        ...
        $.post("<%=basePath%>role/adduserrole", {
            "pguid": $("#master_hidpguid").val(),
            "rcodestr": pid
        }, function (data, status) {
            if (status == "success") {
