        ...
        <div class="layui-card-body">
            <div class="layui-form" lay-filter="">
                <div class="layui-form-item">
                    ...
                    <img src="${person.logo}" style="width: 79px;height: 79px;margin-left: 15px;">
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">手机号</label>
                    <label class="layui-form-label" style="width: auto;">${person.loginname}
                    </label>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">昵称</label>
                    <label class="layui-form-label" style="width: auto;">${person.displayname}</label>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">公司名称</label>
                    <label class="layui-form-label" style="width: auto;">${person.company}</label>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">邮箱</label>
                    <label class="layui-form-label" style="width: auto;">${person.email}</label>
                </div>
                <div class="layui-form-item layui-form-text">
                    <label class="layui-form-label">身份证</label>
                    <label class="layui-form-label" style="width: auto;">${person.sfz}</label>
                </div>
        ...
        <div class="layui-card-body">
            <div class="layui-form" lay-filter="">
                <div class="layui-form-item layui-form-text">
                    ...
                    <div class="layui-input-block" style="padding-left: 10px;">
                        <input type="radio" name="auth" value="1" title="通过"
                               <c:if test="${person.auth=='1'}">checked</c:if> disabled/>
                ...
                <div class="layui-form-item layui-form-text">
                    ...
                    <div class="layui-input-block" style="padding-left: 10px;">
                        <input type="radio" name="cata" value="0" title="管理员"
                               <c:if test="${person.cata=='0'}">checked</c:if> disabled/>
                        <input type="radio" name="cata" value="1" title="普通用户"
                               <c:if test="${person.cata!='0'}">checked</c:if> disabled/>
                    </div>
                </div>
                <div class="layui-form-item layui-form-text">
                    <input type="hidden" id="hidpguid" value="${person.pguid}"/>
                    <label class="layui-form-label">微信openid</label>
                    <label class="layui-form-label" style="width: auto;">${person.openid}</label>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    layui.use(['layer', 'laydate', 'element', 'form'], function () {
        ...
        form.on('submit(managerup)', function (data) {
            $.ajax({
                async: false, // 使用同步的方法
                type: "post",
                url: '<%=basePath%>manager/validate2',
                data: { //要提交到服务端验证的用户名
    ...
    function edit() {
        {
            $.post("<%=basePath%>manager/getsingle", {"pguid": $("#hidpguid").val()}, function (data, status) {
                if (status == "success") {
                    if (data != "") {
