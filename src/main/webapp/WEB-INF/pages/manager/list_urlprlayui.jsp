...
<script>
    ...
    layui.use(['table', 'layer', 'laydate', 'element'], function () {
        ...
        table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') {
                ...
                $.post("<%=basePath%>urlpr/getsingle", {"sguid": trdata.sguid}, function (data, status) {
                    if (status == "success") {
                        if (data != "") {
    ...
    function urlpradd() {
        ...
        $.post("<%=basePath%>urlpr/add", opts, function (data, status) {
            if (status == "success") {
                if (data == "1") {
    ...
    function urlprup() {
        ...
        $.post("<%=basePath%>urlpr/updatebysguid", opts, function (data, status) {
            if (status == "success") {
                if (data == "1") {
