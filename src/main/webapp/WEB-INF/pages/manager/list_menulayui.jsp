...
<table id="tableax" class="layui-table" style="display: none;">
    ...
    <tr>
        <td>菜单显示顺序:</td>
        <td>
            <select  id="menulist_txtsort" style="width: 100px; height: 25px; border-radius: 5px; border: 1px solid #A4D3EE;">
                <option value=""></option>
...
<script>
    ...
    layui.use(['table', 'layer', 'laydate', 'element','form'], function () {
        ...
        table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') {
                ...
                $.post("<%=basePath%>menu/getsingle", {"mid": trdata.mid}, function (data, status) {
                    if (status == "success") {
                        if (data != "") {
    ...
    function menuadd()
    {
        if($("#perm_txtpname").val()==""){
            layer.msg('请填写权限描述', {icon: 2,time:2000});
            return;
        }
        if($("#perm_txtpurl").val()==""){
            layer.msg('请填写权限url', {icon: 2,time:2000});
            return;
        }
        $.post("<%=basePath%>menu/add",{"cata":$("#menulist_slcata").val(),"menuname":$("#menulist_txtmenuname").val(),"menucode":$("#menulist_txtmenucode").val(),"url":$("#menulist_txturl").val(),"iconname":$("#menulist_txticonname").val(),"sort":$("#menulist_txtsort").val()},function(data,status){
            if (status=="success") {
                if (data=="1") {
    ...
    function menuup()
    {
        ...
        $.post("<%=basePath%>menu/updatebymid",{"mid":$("#menulist_hidpid").val(),"cata":$("#menulist_slcata").val(),"menuname":$("#menulist_txtmenuname").val(),"menucode":$("#menulist_txtmenucode").val(),"url":$("#menulist_txturl").val(),"iconname":$("#menulist_txticonname").val(),"sort":$("#menulist_txtsort").val()},function(data,status){
            if (status=="success") {
                if (data=="1") {
    ...
    function getMenu()
    {
        $.post("<%=basePath%>menu/getmenu",{},function(data,status)
        {
            if (status=="success") {
