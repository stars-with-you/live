...
<script>
    ...
    layui.use(['table', 'layer', 'laydate', 'element'], function () {
        ...
        table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') {
                ...
                $.post("<%=basePath%>role/getsingle", {"rcode": trdata.rcode}, function (data, status) {
                    if (status == "success") {
                        if (data != "") {
    ...
    function roleadd()
    {
        ...
        $.post("<%=basePath%>role/add",{"rname":$("#role_txtrname").val()},function(data,status){
    ...
    function roleup()
    {
        ...
        $.post("<%=basePath%>role/updatebyrcode",{"rname":$("#role_txtrname").val(),"rcode":$("#role_txtcode").val()},function(data,status){
            if (status=="success") {
                if (data=="1") {
    ...
    function roleperm(rcode)
    {
        ...
        $.post("<%=basePath%>perm/getlistbyrcode",{"rcode":rcode},function(data,status){
            if (status=="success") {
                if (data!="") {
                    var json=$.parseJSON(data);
    ...
    function permadd()
    {
        ...
        $.post("<%=basePath%>role/addroleperm",{"rcode":$("#role_hidrcode").val(),"pidstr":pid},function(data,status){
            if (status=="success") {
                if (data=="1") {
    ...
    $(function () {
        //绑定添加权限按钮事件
        $.post("<%=basePath%>perm/getlistall",function(data,status){
            if (status=="success") {
                if (data!="") {
                    var json=$.parseJSON(data);
                    var str="";
